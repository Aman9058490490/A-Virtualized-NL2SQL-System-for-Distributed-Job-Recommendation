...
# SYSTEM_PROMPT = """
# You are an expert federated SQL planner. Decompose user questions into independent SQL queries for two separate MySQL databases.

# CRITICAL RULES:
# - Always return valid JSON with keys: "course_sql", "job_sql", "unstructured_prompt"
# - Each SQL query must be COMPLETELY INDEPENDENT - no cross-database references
# - coursedb SQL can ONLY reference course_details view
# - fe_database SQL can ONLY reference renamed_frontend_engineer table
# - NEVER use database prefixes like "fe_database." or "coursedb."
# - SQL must be MySQL compatible and start with SELECT
# - Add LIMIT 25 for large result sets
# - Use table aliases for clarity when needed
# - Match skills by looking at skills_taught and fe_skills columns
# - For coursedb queries: focus on courses with relevant skills
# - For fe_database queries: focus on jobs with required skills
# - Return empty string if database is not needed for the query
# """.strip()

# SYSTEM_PROMPT = """
# You are an expert federated SQL planner. Decompose the user's natural language
# question into three outputs and return them as a JSON object with keys:
...
# Descriptions:
# - "course_sql": SQL query for CourseDB (software_database) using table
#   `software_engineer_jobs` and columns prefixed with `se_`.
# - "job_sql": SQL query for JobDB (frontend_database) using table
#   `frontend_engineer_jobs` and columns prefixed with `fe_`.
# - "natural_query": A natural-language reasoning prompt intended to be passed
#   to a second LLM AFTER SQL results are retrieved. It should reference the
#   user question + how to use the SQL results. If no higher-level reasoning is
#   needed, return "".

# Rules:
# - SQL must begin with SELECT and be valid MySQL.
# - Use LIMIT 25 for large result sets.
# - Use LOWER() for case-insensitive matching.
# - If either database is not needed, return an empty string for its SQL.
# - Return strictly valid JSON.
# """.strip()

# SYSTEM_PROMPT = """
# You are an expert federated SQL decomposition engine. Your job is to take a
# user's natural language question and break it into:

# 1. "course_sql"       – SQL query for the SOFTWARE database
# 2. "job_sql"          – SQL query for the FRONTEND database
# 3. "natural_query"    – A natural-language reasoning prompt that another LLM
#                         will use AFTER SQL execution to combine and explain
#                         the results.
...
# - All SQL must be valid MySQL and begin with SELECT.
# - Always include LIMIT 25 for larger result sets.
# - Use only table names, never database prefixes.
# - Use LOWER(column) with LIKE for case-insensitive text matching.
# - If a database is NOT required by the user query AND the routing logic,
...
# - "natural_query" should be a plain-English prompt that tells a second LLM
#   how to interpret the SQL results and answer the user's high-level request.
# - The natural query should:
#     • Incorporate the intent of the user's original question
#     • Assume it will receive the SQL results from both DBs
#     • Ask the second LLM to combine, compare, or reason using those results
# - If no reasoning is required, return "".
...
# Make sure the JSON is strictly valid.
# """.strip()

# SYSTEM_PROMPT = """
# You are an expert federated SQL planner. Your job is to convert the user's
# natural language question into THREE outputs:

# 1. "course_sql"      – SQL for the SOFTWARE database
# 2. "job_sql"         – SQL for the FRONTEND database
# 3. "natural_query"   – A natural-language reasoning prompt for a second LLM
#                        that will interpret the SQL results.
...
# You MUST generate SQL for the appropriate database based on the user intent:

# ● For software-related queries → ALWAYS generate course_sql.
#    Includes terms like:
#    - software jobs, software engineer, software developer
#    - backend developer, fullstack developer
#    - general queries about “software roles”

# ● For frontend-related queries → ALWAYS generate job_sql.
#    Includes terms like:
#    - frontend jobs, frontend engineer, UI, UX, JavaScript developer
#    - web developer, react developer, typescript developer

# ● If BOTH software and frontend are mentioned → YOU MUST generate BOTH SQLs.

# ● Only leave a SQL string empty ("") if the database is entirely irrelevant
#   AND the routing logic indicates that it is not required.

# ======================================================================
# STRICT SQL RULES
# ======================================================================

# - SQL must be valid MySQL and always start with SELECT.
# - Use ONLY table names, no database prefixes.
# - ALWAYS use correct prefixes:
#       se_ for software_engineer_jobs
#       fe_ for frontend_engineer_jobs
#   NEVER invent new prefixes like sd_, sf_, s_, etc.
# - Use LOWER(column) LIKE '%text%' for case-insensitive matching.
# - Use LIMIT 25 for large queries.
# - Only use columns that exist in the sample rows or schema.

# ======================================================================
# NATURAL QUERY RULES
# ======================================================================

# - "natural_query" must help a second LLM combine and interpret SQL results.
# - It should restate the user's real intent.
# - It should reference BOTH software and frontend results if both appear.
# - Keep it clear, concise, and focused on reasoning.
...
class QueryAnalyzer:
    ...
    def generate_decomposition(self, query: str) -> AnalyzerResult:
        ...
        for attempt in range(1, self._max_attempts + 1):
            ...
            # unstructured_prompt = (payload.get("unstructured_prompt", "") or "").strip()
            course_sql = normalize_sql_whitespace(payload.get("course_sql", ""))
            job_sql = normalize_sql_whitespace(payload.get("job_sql", ""))
            ...
            if (course_sql or job_sql) and not (missing_course or missing_job):
                ...
                #     job_sql=job_sql,
                #     unstructured_prompt=unstructured_prompt,
                # )
                return AnalyzerResult(
                    course_sql=course_sql,
                    job_sql=job_sql,
    ...
    def _fallback_decomposition(self, query: str) -> AnalyzerResult:
        ...
        # return AnalyzerResult(course_sql=course_sql, job_sql=job_sql, unstructured_prompt=prompt)
